...
with st.sidebar:
    ...
    iep_host_name = st.sidebar.text_input(
        "Enter the Inference Endpoint URL",
        disabled=st.session_state.iep_host_disabled,
        on_change=iep_host_disable,
    )

    st.subheader("Endpoint Configuration")
    endpoint_name = st.sidebar.text_input(
        "Enter the Endpoint name",
        disabled=st.session_state.endpoint_disabled,
        on_change=endpoint_disable,
    )
    endpoint_api_key = st.sidebar.text_input(
        "Enter the Endpoint API key",
        disabled=st.session_state.apikey_disabled,
        on_change=apikey_disable,
        type="password",
    )

    if (
        "iep_host_name" in st.session_state
        and st.session_state["iep_host_name"] != iep_host_name
    ):
        clear_chat_history()

    if (
        "endpoint_name" in st.session_state
        and st.session_state["endpoint_name"] != endpoint_name
    ):
        clear_chat_history()

    if (
        "endpoint_api_key" in st.session_state
        and st.session_state["endpoint_api_key"] != endpoint_api_key
    ):
        clear_chat_history()
...
client = OpenAI(
    base_url=iep_host_name.removesuffix("/chat/completions"),
...
if prompt := st.chat_input(
    st.session_state.chat_default, disabled=st.session_state.chat_disabled
):
    ...
    try:
        ...
        print(e)
        st.error(
            "Error. Did you set Inference Endpoint host name, Endpoint name and API key correctly?"
        )
