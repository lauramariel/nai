...
do
PAYLOAD=$(cat <<EOF
{"email":"$USER_NAI_NAME@ntnxlab.local","password":"$NAI_DEFAULT_PW","firstName":"Bootcamp","lastName":"User$(cat /dev/urandom | LC_ALL=C tr -dc 'A-Za-z' | head -c 5)","username":"$USER_NAI_NAME","role":"MLUser"}
EOF
)
